...
## Keeping Key Variables Using Data Visualizations

### Numerical Variables:

#### Default vs Non Default Distributions

The boxplots below show if the distribution of each feature differs in default
...
The correlation heatmap below shows pearson correlations between the numeric
variables in the data. Excluding the obvious diagonal, no variable relationship
is over than the 0.7 threshold of multicollinearity (no higher than 0.5 in the
heatmap).
...
The Bar chart below shows which categorical variables are associated with
...
`prop_valuation`, `or_loan_term`, `interest_only`, `ppm`, and `upd` have been
shown to be weak features so they will not be included in our model
...
final_2018.to_csv("data/final_2018.csv", index=True)
```
...
## Performance Data Cleaning Function:
...
**(create autoated pipeline for merging origination and performance datasets.
final clean output to be fed to model for use by flask app)**
